...
pub fn read_u16(f: &mut Read) -> u16 {
    let mut buf = [0u8; 2];
...
pub fn read_u32(f: &mut Read) -> u32 {
    let mut buf = [0u8; 4];
...
pub fn read_vec_u8(f: &mut Read, length: usize) -> Vec<u8> {
...
pub fn read_vec_u16(f: &mut Read, length: usize) -> Vec<u16> {
    use std::slice;
